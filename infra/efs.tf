...
resource "aws_security_group" "allow-efs" {
  ...
  description = "Allow EFS acces for EKS cluster."
  ...
  ingress {
    ...
    tags = {
        Name = "Allow EFS acces for ${local.cluster_name}" 
    }
}

    
module "efs_csi_isra_role" {
    ...
    oidc_providers = {
        one = {
            ...
            namespace_service_accounts = ["kube-system:efs-csi_controller-sa"]
        }
    }
}